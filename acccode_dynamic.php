...
if (isset($_REQUEST['sub1'])) {
    ...
    $sql="
        ...
        FROM test2 t
        WHERE t.user = 'acc' AND t.category = 'finance' AND t.factor > 3
...
if (isset($_REQUEST['sub2'])) {
    ...
    $sql="
        SELECT t.title, t.link, t.image
        FROM test2 t
        WHERE t.user = 'acc' AND t.category = 'taxes' AND t.factor > 3
        ORDER BY t.factor ASC
        LIMIT 8";
    $query= mysqli_query($con, $sql);
    while ($r= mysqli_fetch_assoc($query)){
...
if (isset($_REQUEST['sub3'])) {
    ...
    $sql="
        SELECT t.title, t.link, t.image
        FROM test2 t
        WHERE t.user = 'acc' AND t.category = 'law' AND t.factor > 3
        ORDER BY t.factor ASC
        LIMIT 8";
    $query= mysqli_query($con, $sql);
    while ($r= mysqli_fetch_assoc($query)){
